...
namespace DevionGames
{
	public abstract class MotionState : MonoBehaviour
	{
        ...
        [SerializeField, Tooltip("Reference to the active ThirdPersonCamera.")]
		...
		public void StopMotion (bool force)
        {
            ...
            if (this.m_Camera != null && this.m_Camera.Presets != null)
            {
            ...
            {
                Debug.LogWarning($"[MotionState] m_Camera or Presets list is null on {gameObject.name} during StopMotion.");
            }
        ...
        public void StartMotion()
        {
            ...
            // Ensure m_Camera and its Presets list are valid
            if (this.m_Camera != null && this.m_Camera.Presets != null)
            {
                CameraSettings preset = this.m_Camera.Presets.FirstOrDefault(x => x.Name == CameraPreset);
                if (preset != null)
            ...
            {
                Debug.LogWarning($"[MotionState] m_Camera or Presets list is null on {gameObject.name}");
            }

            // Ensure animator is valid and transition state is non-empty
            ...
            if (!string.IsNullOrEmpty(destinationState))
            {
                ...
                {
                    Debug.LogWarning($"[MotionState] m_Animator is null on {gameObject.name}. Cannot play '{destinationState}'.");
                }
            }
