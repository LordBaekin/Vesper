...
namespace DevionGames.InventorySystem
{
    public class PlaceItem : MonoBehaviour
    {
		...
		private void Start()
		{
			...
			this.m_Player = InventoryManager.Current.GetPlayerInfo().transform;
		...
		private float GetMaxCornerHeight()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				ext.Scale(new Vector3((i & 1) == 0 ? 1 : -1, -1, (i & 2) == 0 ? 1 : -1) * 0.5f);
				Vector3 vertPositionLocal = boxColliderCenter + ext;
                if (Physics.Raycast(transform.TransformPoint(vertPositionLocal) + Vector3.up, Vector3.down, out RaycastHit hit, Mathf.Infinity, mask))
                {
