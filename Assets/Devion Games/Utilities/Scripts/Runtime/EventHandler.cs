...
namespace DevionGames{
	public class EventHandler : MonoBehaviour {
		...
		{
            if (!EventHandler.m_GlobalEvents.TryGetValue(eventName, out Delegate mDelegate))
            {
		...
		{
			if (obj == null) return;
            if (!EventHandler.m_Events.TryGetValue(obj, out Dictionary<string, Delegate> mEvents))
            {
		...
		{
			if (obj == null) return;
            if (EventHandler.m_Events.TryGetValue(obj, out Dictionary<string, Delegate> mEvents) && mEvents.TryGetValue(eventName, out Delegate mDelegate))
            {
